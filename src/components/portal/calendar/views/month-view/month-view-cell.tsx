...
export const MonthViewCell = ({ dayIndex, date, isCurrentMonth }: MonthCalendarDayCellProps) => {
  const dayKey = useMemo(() => getDayKeyFromDate(date), [date]);

  const { selectedDate, layoutOccurrences, updateLayoutCache } = useCalendarStore(
    useShallow((store) => ({
  ...
  const [visibleOccurrences, setVisibleOccurrences] = useState<number>(totalOccurrences);
  const [isViewAllOpen, setIsViewAllOpen] = useState(false);

  useEffect(() => {
    const observer = new ResizeObserver((entries) => {
      const entry = entries[0] as ResizeObserverEntry;
      const sessionContainerHeight = entry.target.getBoundingClientRect().height;

      const visibleSessions = Math.floor(
        sessionContainerHeight / (SESSION_OCCURRENCE_HEIGHT + SESSION_OCCURRENCE_SPACING)
      );
  ...
  return (
    ...
    >
      ...
      <div ref={sessionsContainerRef} className='mt-1 overflow-hidden grow p-1 w-full space-y-1'>
        ...
        {visibleOccurrences < totalOccurrences && (
          <Popover isOpen={isViewAllOpen} onOpenChange={(open) => setIsViewAllOpen(open)}>
            ...
            <Popover.Content placement='top' className='w-[300px] relative overflow-visible'>
              <Popover.Close
                appearance='plain'
                size='square-petite'
              ...
              <Popover.Header className='flex flex-col items-center justify-center'>
                <p className='text-sm text-text-sub lowercase'>{getWeekdayAbbrev(date)}</p>
                <p className='text-3xl font-medium text-responsive-dark '>{date.day}</p>
              </Popover.Header>

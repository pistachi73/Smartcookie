import { ArrowRight02Icon, Clock02Icon } from "@hugeicons-pro/core-solid-rounded";
...
import { CalendarStoreContext, useCalendarStore } from "@/providers/calendar-store-provider";
...
export const UpcomingEvents = () => {
  ...
  const layoutOccurrences = useCalendarStore((state) => state.getLayoutOccurrences(todayKey));
...
const UpcomingOccurrenceCard = ({ occurrence }: { occurrence: UIOccurrence }) => {
  ...
  return (
    ...
    >
      <div
        className={cn("my-1 rounded-lg w-1 shrink-0 min-w-0 min-h-0 border", color?.className)}
      />
      <div className={cn("text-left flex flex-col gap-0 justify-between py-2.5")}>
